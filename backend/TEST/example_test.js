...
describe('AddTask Function Test', () => {
  ...
  it('should create a new task successfully', async () => {
    ...
    // Assertions - verify if it behaves as expected
    expect(createStub.calledOnceWith({ userId: req.user.id, ...req.body })).to.be.true;
    expect(res.status.calledWith(201)).to.be.true;
    expect(res.json.calledWith(createdTask)).to.be.true;

    // Restore stubbed methods - clean up or reset any mocked or stubbed methods or functions after a test case has run
    createStub.restore();
  });
